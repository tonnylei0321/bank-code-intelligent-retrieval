...
async def test_specific_query(question):
    db = next(get_db())
    try:
        ...
        for i, result in enumerate(results):
            ...
            method = result.get('retrieval_method', 'vector')
            print(f"  {i+1}. {result['bank_name']} -> {result['bank_code']} (分数: {score:.3f}, 方法: {method})")
...
async def main():
    ...
    for query in queries:
        await test_specific_query(query)
        print()
